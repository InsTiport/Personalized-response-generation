...
class Bart_wiki_model(nn.Module):
    def __init__(self, vocab_size=50265, hidden_dim=768):
        super().__init__()

        self.hidden_dim = hidden_dim

        self.bart = BartModel.from_pretrained('facebook/bart-base')
        self.linear = nn.Linear(3 * hidden_dim, vocab_size)
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids, decoder_attention_mask, section_wiki_encoding, game_wiki_encoding):
        ...
        section_wiki_encoding = section_wiki_encoding.unsqueeze(1).expand(batch_size, out_seq_len, self.hidden_dim).to(input_ids.device)
        game_wiki_encoding = game_wiki_encoding.unsqueeze(1).expand(batch_size, out_seq_len, self.hidden_dim).to(input_ids.device)
