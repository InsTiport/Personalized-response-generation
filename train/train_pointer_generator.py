...
MODEL_NAME = f'pointer-generator_bsz_{BATCH_SIZE}'
log_file = open(os.path.join('model_weights', f'{MODEL_NAME}.log'), 'w')

print(f'Training pointer-generator model for {NUM_EPOCH} epochs, with batch size {BATCH_SIZE}')
...
# load model
model = PointerGenerator().to(device)
