...
MODEL_NAME = f'bart-wiki-concat_bsz_{BATCH_SIZE}_seed_{SEED}'
log_file = open(os.path.join('model_weights', f'{MODEL_NAME}.log'), 'w')

print(f'Training BART wiki with concatenation for {NUM_EPOCH} epochs, with batch size {BATCH_SIZE}')
...
for epo in range(NUM_EPOCH):
    ...
    for batch in train_iterator_with_progress:
        ...
        inputs = [wiki + tokenizer.sep_token + q for q, wiki in zip(batch_q, batch_wiki)]
