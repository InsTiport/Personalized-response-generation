...
with torch.no_grad():
    with open(os.path.join('data', 'interviewee.csv')) as r:
        ...
        lines = r.readlines()
        while counter < 30:
            rand_line_number = (random.randrange(1, 17285))
            line = lines[rand_line_number].rstrip()

            if 'football' not in line:
                continue

            counter += 1
            print(f'Line: {line}')

            question = [args.before + line[:line.index('_')] + args.after + '?']

            print(f'Question asked: {question[0]}')

            # input encoding
            input_encoding = tokenizer(question, return_tensors='pt', padding=True, truncation=True).to(device)
